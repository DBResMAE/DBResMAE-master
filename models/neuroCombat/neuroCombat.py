...
def neuroCombat(dat,
    ...
    print("covar_labels:", covar_labels)
    ...
    print("batch_levels:", batch_levels, "sample_per_batch:", sample_per_batch)
...
def make_design_matrix(Y, batch_col, cat_cols, num_cols, ref_level):
    """
    Return Matrix containing the following parts:  返回矩阵包含以下部分:
        - one-hot matrix of batch variable (full)  批变量独热矩阵(满)
        - one-hot matrix for each categorical_cols (removing the first column)  每个categorical_cols一个独热矩阵(删除第一列)
        - column for each continuous_cols  每个continuous_cols的列
    """
    ...
    print("batch_onehot:",batch_onehot)
...
def standardize_across_features(X, design, info_dict):
    ...
    count = 0
    def get_beta_with_nan(yy, mod, count):
        wh = np.isfinite(yy)
        if count == 0:
            print("wh:",wh)
        mod = mod[wh,:]
    ...
    for i in range(X.shape[0]):
        if i >0:
            count = 1
        betas.append(get_beta_with_nan(X[i,:], design, count))
    ...
    if ref_level is not None:
        ...
        grand_mean = np.dot((sample_per_batch/ float(n_sample)).T, B_hat[:n_batch,:]) #sample_per_batch:[5 5]  n_sample:10    n_batch:2
        print("sample_per_batch:", sample_per_batch) #shape(1,200)
...
def adjust_data_final(s_data, design, gamma_star, delta_star, stand_mean, mod_mean, var_pooled, info_dict, dat):
    ...
    bayesdata = bayesdata * np.dot(vpsq, np.ones((1, n_sample))) + stand_mean + mod_mean   # 最后一个协调公式
