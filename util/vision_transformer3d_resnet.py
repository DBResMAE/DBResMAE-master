...
class AdaptiveInstanceNorm(nn.Module):
    def __init__(self, in_channel, style_dim, batch_size):
        super().__init__()
    ...
    def forward(self, input):
        ...
        out = self.norm(input) #(batch, num_patches, dim)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.AdaIN1 = AdaptiveInstanceNorm(dim, dim, self.batch_size)
        self.AdaIN2 = AdaptiveInstanceNorm(dim, dim, self.batch_size)
        self.attn = Attention(
    ...
    def forward(self, x):
        x = x + self.drop_path(self.attn(self.AdaIN1(x)))
        x = x + self.drop_path(self.mlp(self.AdaIN2(x)))
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(112, 128, 112), patch_size=16, in_chans=1, num_classes=2, embed_dim=768, depth=12,
        ...
        self.blocks2 = nn.ModuleList([
            Block(embed_dim // 8, 6, mlp_ratio, qkv_bias=True, qk_scale=None,
                  drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
            for i in range(6)])
        ...
        self.blocks3 = nn.ModuleList([
            Block(embed_dim, 2, mlp_ratio, qkv_bias=True, qk_scale=None,
                  drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
            for i in range(2)])
    ...
    def x2_reshape(self, x2, B):
        # 初始化一个空的结果张量，大小为（7，8，7，96*8）
        device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
        result_tensor = torch.zeros(B, 7, 8, 7, 96 * 8,device=device)
