...
# --------------------------------------------------------
def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, flag=False):
    """
    grid_size: int of the grid height and width
    return:
    pos_embed: [grid_size*grid_size, embed_dim] or [1+grid_size*grid_size, embed_dim] (w/ or w/o cls_token)
    """
    grid_h = np.arange(7, dtype=np.float32)
    grid_w = np.arange(8, dtype=np.float32)
    grid_d = np.arange(7, dtype=np.float32)
    grid = np.meshgrid(grid_d, grid_w, grid_h)  # here w goes first
    grid = np.stack(grid, axis=0)

    grid = grid.reshape([3, 1, 7, 8, 7])
...
#         emb_d = get_1d_sincos_pos_embed_from_grid(embed_dim // 3, grid[2])  # (H*W, D/2)
#     else:
#         emb_h = get_1d_sincos_pos_embed_from_grid(embed_dim // 3 -1, grid[0])  # (H*W, D/2)
#         emb_w = get_1d_sincos_pos_embed_from_grid(embed_dim // 3+3, grid[1])  # (H*W, D/2)
#         emb_d = get_1d_sincos_pos_embed_from_grid(embed_dim // 3-1, grid[2])  # (H*W, D/2)
#
#     emb = np.concatenate([emb_h, emb_w, emb_d], axis=1) # (H*W, D)
#     return emb
