...
import model_vit_Baseline_SPEB
...
for k in range(0, 10):
    ...
    with open('/home/manjianzhi/jinjin/MAE/Baseline_SPEB_images/ADNI_test_fake_datas_'+str(k+1)+'.pkl', 'rb') as f:
    ...
    mae = model_vit_Baseline_SPEB.vit_base_patch16()
    ...
    # load model
    mae.load_state_dict(torch.load('./weights/SPEB_10fold/Baseline_SPEB_wegihts_fold_'+str(k+1)+'.pth'), strict=False)
    ...
    torch.save(mae.state_dict(), './weights/finetune_Baseline_SPEB/finetune_fold_'+str(k+1)+'.pth')
...
print("Baseline_SPEB模型10折病理分类结果：")
print("训练集：")
print(train_acc)
print("平均分类准确率：")
